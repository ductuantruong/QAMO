...
class Model(nn.Module):
    def __init__(self, args, device):
        ...
        self.Nested_Res2Net_TDNN = Nested_Res2Net_TDNN(Nes_ratio=Nes_ratio,
                                                       input_channel=1024,
                                                       n_output_logits=n_output_logits,
                                                       dilation=dilation,
                                                       pool_func=pool_func,
                                                       SE_ratio=SE_ratio)
        if pool_func == 'mean':
            self.ocsoftmax = QAMO(1024)
        elif pool_func == 'ASTP':
            self.ocsoftmax = QAMO(2048)

    def forward(self, x, y=None, q_score=None):
