...
def evaluate_accuracy(dev_loader, model, device, epoch):
    val_loss = 0.0
    num_total = 0.0
    correct=0
    model.eval()
    ...
    i=0
    with torch.no_grad():
      for batch_x, batch_y, batch_qscore, utt_id in dev_loader:
        batch_size = batch_x.size(0)
        num_total += batch_size
        batch_x = batch_x.to(device)
        batch_qscore = batch_qscore.long().to(device)
        batch_y = batch_y.view(-1).type(torch.int64).to(device)
        _, batch_loss, batch_out = model(batch_x, batch_y)
...
def produce_evaluation_file(dataset, model, device, save_path):
    ...
    del fname_list
    del ens_score_list
    with open(save_path, 'a+') as ens_fh:
        ens_fh.write('\n'.join(ens_text_list) + '\n')
    print('Scores saved to {}'.format(save_path))
    del ens_text_list
    ens_fh.close()

def train_epoch(train_loader, model, lr, optim_head, optim_back, device, epoch):
    num_total = 0.0
    num_spoof = 0
    num_good_spoof = 0
    num_bonafide = 0
    ...
    for i, batch in enumerate(pbar):
        batch_x, batch_y, batch_qscore, utt_id = batch

        batch_size = batch_x.size(0)
        num_total += batch_size
        num_good_spoof += batch_qscore[batch_y==1].sum()
        num_good_bonafide += batch_qscore[batch_y==0].sum()
        num_spoof += (batch_y==1).sum()
        num_bonafide += (batch_y==0).sum()

        batch_x = batch_x.to(device)
        batch_qscore = batch_qscore.long().to(device)
        batch_y = batch_y.view(-1).type(torch.int64).to(device)
        _, batch_loss, batch_out = model(batch_x, batch_y, batch_qscore)

        batch_cls_loss, batch_out_quality = batch_loss
        if batch_qscore[batch_y==1].size(0) == 0:
            batch_quality_loss = torch.tensor(0.).to(device)
        else:
            batch_quality_loss = criterion(batch_out_quality, batch_qscore[batch_y==1])

        pbar.set_description(f"Epoch {epoch}: batch_cls_loss {batch_cls_loss.item()} batch_quality_loss {batch_quality_loss.item()}")
        optim_head.zero_grad()
        optim_back.zero_grad()
        total_batch_loss = batch_cls_loss + batch_quality_loss * 0.1
        total_batch_loss.backward()
        optim_head.step()
        optim_back.step()
        i=i+1
    print(f"Epoch {epoch}: No. good bonafide / Total bonafide: {num_good_bonafide} / {num_bonafide}")
    print(f"Epoch {epoch}: No. good spoof / Total spoof: {num_good_spoof} / {num_spoof}")
    sys.stdout.flush()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--oc_lr', type=float, default=1e-3)
    ...
    #set Adam optimizer
    oc_params = [p for name, p in model.named_parameters() if 'ocsoftmax' in name]
    backbone_params = [p for name, p in model.named_parameters() if 'ocsoftmax' not in name]

    print('oc_params', len(oc_params))
    print('backbone_params', len(backbone_params))
    optim_backend = torch.optim.Adam(backbone_params, lr=args.lr, weight_decay=args.weight_decay)
    optim_head = torch.optim.Adam(oc_params, lr=args.oc_lr, weight_decay=args.weight_decay)

    # define train dataloader
    label_trn, files_id_train, score_trn = read_metadata( dir_meta =  os.path.join(args.protocols_path+'LA/{}_cm_protocols/{}.cm.train.trn.score.txt'.format(prefix,prefix_2019)), is_eval=False)
    print('no. of training trials',len(files_id_train))
    
    train_set=Dataset_train(args,list_IDs = files_id_train,labels = label_trn, scores = score_trn, base_dir = os.path.join(args.database_path+'LA/{}_{}_train/'.format(prefix_2019.split('.')[0],args.track)),algo=args.algo)
    train_loader = DataLoader(train_set, batch_size=args.batch_size, num_workers = 10, shuffle=True,drop_last = True)
    
    # define validation dataloader
    labels_dev, files_id_dev, score_dev = read_metadata( dir_meta =  os.path.join(args.protocols_path+'LA/{}_cm_protocols/{}.cm.dev.trl.score.txt'.format(prefix,prefix_2019)), is_eval=False)
    print('no. of validation trials',len(files_id_dev))

    dev_set = Dataset_train(args,list_IDs = files_id_dev, labels = labels_dev, scores = score_dev, base_dir = os.path.join(args.database_path+'LA/{}_{}_dev/'.format(prefix_2019.split('.')[0],args.track)),
                            algo=0)
    ...
    if args.train:
        ...
        while not_improving<args.num_epochs:
            ...
            with torch.no_grad():
                embeddings = F.normalize(model.ocsoftmax.center, p=2, dim=1)
                sim_matrix = torch.matmul(embeddings, embeddings.T)
                print(sim_matrix)
            train_epoch(train_loader, model, args.lr, optim_head, optim_backend, device, epoch)
    ...
    if not os.path.exists(os.path.join(model_save_path, 'oc_center.npy')):
        np.save(os.path.join(model_save_path, 'oc_center.npy'), model.ocsoftmax.center.cpu().detach().numpy())
